 ...
 */
  ...
  $content .= '<div id="' . str_replace('_', '-', $box_id . 'Content') . '" class="sideBoxInnerContent offset-top-10">';
  ...
  $content .= '<div id="cartBoxListWrapper">' . "\n" . '<ul class="simple-list">' . "\n";
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      ...
      $content .= '<a href="' . zen_href_link(zen_get_info_page($products[$i]['id']), 'products_id=' . $products[$i]['id'], 'SSL') . '"> ' . $products[$i]['quantity'] . '&nbsp;-&nbsp;';
  ...
  if ($_SESSION['cart']->count_contents() > 0) {
    //$content .= '<hr />';
    $content .= '<div class="cartBoxTotal shopping-cart-table__product-price unit-price"> Total : ' . $currencies->format($_SESSION['cart']->show_total()) . '</div>';
    $content .= '';
  }
