...
<div id="productListing" class="product-listing carousel-products-mobile row <?php echo $grid_list_class; ?>">
	<div id="product-area" <?php echo $newprodpage_container_data; ?>>
...
<?php
	...
	if($products_new_split->number_of_rows > 0) {  
		...
		while (!$products_new->EOF) {
			...
			//set Infopagelink
			$zen_get_info_page = zen_get_info_page($products_new->fields['products_id']);
			...
			if (PRODUCT_NEW_LIST_DATE_ADDED != '0' and zen_get_show_product_switch($products_new->fields['products_id'], 'date_added')) {
				$display_products_date_added =  sprintf(TEXT_DATE_ADDED, zen_date_long($products_new->fields['products_date_added'])) . str_repeat('', substr(PRODUCT_NEW_LIST_DATE_ADDED, 3, 1));
			}
			...
			if ((PRODUCT_NEW_LIST_PRICE != '0' and zen_get_products_allow_add_to_cart($products_new->fields['products_id']) == 'Y') and zen_check_show_prices() == true) {
				...
				$display_products_price = $products_price . str_repeat('', substr(PRODUCT_NEW_LIST_PRICE, 3, 1)) . (zen_get_show_product_switch($products_new->fields['products_id'], 'ALWAYS_FREE_SHIPPING_IMAGE_SWITCH') ? (zen_get_product_is_always_free_shipping($products_new->fields['products_id']) ? TEXT_PRODUCT_FREE_SHIPPING_ICON : '') : '');
			}
			...
			if (PRODUCT_NEW_BUY_NOW != '0' and zen_get_products_allow_add_to_cart($products_new->fields['products_id']) == 'Y') {
				if (zen_has_product_attributes($products_new->fields['products_id'])) {
					$link = '<a class="list-more btn btn--ys btn--xl" href="' . zen_href_link($zen_get_info_page, 'cPath=' . $cPath . '&products_id=' . $products_new->fields['products_id']) . '" '.pzenExtraBtnLink($products_new).'><span class="icon icon-shopping_basket"></span>' . TITLE_SELECT_OPTIONS . '</a>';
				} else {
					if (PRODUCT_NEW_LISTING_MULTIPLE_ADD_TO_CART > 0 && $products_new->fields['products_qty_box_status'] != 0) {
					//            $how_many++;
						$link = '<div class="prod-qty-bx"><div class="inner-qty-box"><span class="qty-lbl">'.TEXT_PRODUCT_NEW_LISTING_MULTIPLE_ADD_TO_CART . "</span><span class='qty_txt'><input type=\"text\" name=\"products_id[" . $products_new->fields['products_id'] . "]\" value=\"0\" size=\"4\" /></span>".'</div></div>';
					} else {
						$link = '<a class="btn btn--ys btn--xl" href="' . zen_href_link(FILENAME_PRODUCTS_NEW, zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $products_new->fields['products_id']) . '" '.pzenExtraBtnLink($products_new).'><span class="icon icon-shopping_basket"></span>' . zen_image_button(BUTTON_IMAGE_BUY_NOW, BUTTON_BUY_NOW_ALT) . '</a>&nbsp;';
					}
				}

				$the_button = $link;
				if($products_new->fields['product_is_call'] != '1'){
					$products_link = '<a href="' . zen_href_link($zen_get_info_page, 'cPath=' . $cPath . '&products_id=' . $products_new->fields['products_id']) . '">' . MORE_INFO_TEXT . '</a>';
				}
				//if not out of stock
				if($products_new->fields['products_quantity'] > 0 || SHOW_PRODUCTS_SOLD_OUT_IMAGE == 0){
					if((zen_get_products_allow_add_to_cart($products_new->fields['products_id']) != 'N') && $products_new->fields['product_is_call'] == '1'){
				...
				$link = '<a class="list-more btn btn--ys btn--xl" href="' . zen_href_link($zen_get_info_page, 'cPath=' . $cPath . '&products_id=' . $products_new->fields['products_id']) . '">' . MORE_INFO_TEXT . '</a>';
				$the_button = $link;
				$products_link = '<a class="list-more btn btn--ys btn--xl" href="' . zen_href_link($zen_get_info_page, 'cPath=' . $cPath . '&products_id=' . $products_new->fields['products_id']) . '">' . MORE_INFO_TEXT . '</a>';
		...
		$product_content = pzen_get_product_content($products_new, array("image_width"=>IMAGE_PRODUCT_NEW_LISTING_WIDTH, "image_height"=>IMAGE_PRODUCT_NEW_LISTING_HEIGHT));	
	?>
	<div class="<?php echo $newprodpage_item_class; ?>">
    	<div data-filter="all products" data-name="<?php echo $products_name; ?>" class="product <?php echo ($prodlist_image_effects==2)? 'product--zoom' : ''; ?> <?php echo ($prodlistview_image_layout==2)? 'small-size' : ''; ?> <?php echo $product_content['products_class']; ?>">
			<div class="product__inside">
				...
				<?php echo (($product_content['product_new']==1)? '<div class="product__label product__label--right product__label--new"> <span>'.PZEN_BADGE_NEW.'</span></div>' : ''); ?>
