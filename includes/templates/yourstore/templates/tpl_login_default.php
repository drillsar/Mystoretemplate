...
<div class="centerColumn" id="loginDefault">
	<div class="title-box">
		<h2 id="loginDefaultHeading" class="title-under text-uppercase text-center"><?php echo HEADING_TITLE; ?></h2>
	</div>
	...
	<div class="row split-login">
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 create-account">
        	<div class="login-form-box">
				<h3 class="color small"><?php echo HEADING_NEW_CUSTOMER_SPLIT; ?></h3>
				...
				<?php echo zen_draw_form('create', zen_href_link(FILENAME_CREATE_ACCOUNT, (isset($_GET['gv_no']) ? '&gv_no=' . preg_replace('/[^0-9.,%]/', '', $_GET['gv_no']) : ''), 'SSL')); ?>
					<div class="buttonRow forward create-account-button"><?php echo zen_image_submit(BUTTON_IMAGE_CREATE_ACCOUNT, BUTTON_CREATE_ACCOUNT_ALT, 'name="registrationButton"'); ?></div>
				</form>
			</div>
    	</div>
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 login">
    		<div class="login-form-box">
                <h3  class="color small"><?php echo HEADING_RETURNING_CUSTOMER_SPLIT; ?></h3>
				...
				<?php echo zen_draw_form('login', zen_href_link(FILENAME_LOGIN, 'action=process' . (isset($_GET['gv_no']) ? '&gv_no=' . preg_replace('/[^0-9.,%]/', '', $_GET['gv_no']) : ''), 'SSL'), 'post', 'id="loginForm"'); ?>
					...
					<?php echo zen_draw_input_field('email_address', '', 'size="30" id="login-email-address" class="form-control" autofocus placeholder="' . ENTRY_EMAIL_ADDRESS_TEXT . '"' . ((int)ENTRY_EMAIL_ADDRESS_MIN_LENGTH > 0 ? ' required' : ''), 'email'); ?>
					<label class="inputLabel" for="login-password"><?php echo ENTRY_PASSWORD; ?></label>
					<?php echo zen_draw_password_field('password', '', 'size="30" id="login-password" class="form-control" autocomplete="off" placeholder="' . ENTRY_REQUIRED_SYMBOL . '"' . ((int)ENTRY_PASSWORD_MIN_LENGTH > 0 ? ' required' : '')); ?>
		...
		<div class="content_box">
			<fieldset>
				...
				<div class="login-emailaddress">
					<label class="inputLabel" for="login-email-address"><?php echo ENTRY_EMAIL_ADDRESS; ?></label>
					<?php echo zen_draw_input_field('email_address', '', zen_set_field_length(TABLE_CUSTOMERS, 'customers_email_address', '40') . ' id="login-email-address" class="form-control" autofocus placeholder="' . ENTRY_EMAIL_ADDRESS_TEXT . '"' . ((int)ENTRY_EMAIL_ADDRESS_MIN_LENGTH > 0 ? ' required' : ''), 'email'); ?>	
				</div>
				<div class="login-password">
					<label class="inputLabel" for="login-password"><?php echo ENTRY_PASSWORD; ?></label>
					<?php echo zen_draw_password_field('password', '', zen_set_field_length(TABLE_CUSTOMERS, 'customers_password', 40) . ' id="login-password" class="form-control" autocomplete="off" placeholder="' . ENTRY_REQUIRED_SYMBOL . '"' . ((int)ENTRY_PASSWORD_MIN_LENGTH > 0 ? ' required' : '')); ?>
