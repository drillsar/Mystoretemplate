...
// test if box should display
  ...
  if ($show_best_sellers == true) {
    $limit = (trim(MAX_DISPLAY_BESTSELLERS) == "") ? "" : " LIMIT " . (int)MAX_DISPLAY_BESTSELLERS;
      ...
      $best_sellers_query = "select distinct p.products_image, p.products_price, p.products_id, pd.products_name, p.products_ordered
...
if ($best_sellers->RecordCount() >= MIN_DISPLAY_BESTSELLERS) {
      ...
      while (!$best_sellers->EOF) {
		...
		$bestsellers_list[$rows]['products_price']  = $best_sellers->fields['products_price'];
