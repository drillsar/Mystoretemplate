...
if (!empty($_SESSION['compare'])) {
    ...
    foreach ($_SESSION['compare'] as $value) {
        if (!empty($value)) {
            ...
            $products_price = '<div class="shopping-cart-table__product-price unit-price">'.((zen_has_product_attributes_values($products_compare->fields['products_id']) and $flag_show_product_info_starting_at == 1) ? TEXT_BASE_PRICE : '') . zen_get_products_display_price($products_compare->fields['products_id']).'</div>';
...
.'</a>';
			$addtocart = zen_get_buy_now_button($products_compare->fields['products_id'],'<a href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $products_compare->fields['products_id'], 'SSL') . '"><button class="btn btn--ys" type="button"><span> '.COMPARE_BUTTON_ADD_TO_CART.' </span></button></a>');
