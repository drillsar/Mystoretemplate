...
if ($num_categories > 0) {
  ...
  while (!$categories->EOF) {
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="subcategory-item col-lg-4 col-sm-4 col-xs-6 col-xl-one-fifth"',
                                           'text' => '<a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new, 'SSL') . '"><span class="figure">' . zen_image(DIR_WS_IMAGES . $categories->fields['categories_image'], $categories->fields['categories_name'], SUBCATEGORY_IMAGE_WIDTH, SUBCATEGORY_IMAGE_HEIGHT, "class='img-responsive'").'</span><h3 class="subcategory-item__title">'. $categories->fields['categories_name'] . '</h3></a>'/*'text' => '<a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new, 'SSL') . '">' . $categories->fields['categories_name'] . '</a>'*/
										   );
	$col ++;
