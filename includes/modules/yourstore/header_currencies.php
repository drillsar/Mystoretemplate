	...
	if ($show_currencies == true) {
		if (isset($currencies) && is_object($currencies)) {
			...
			while (list($key, $value, $title) = each($currencies->currencies)) {
				$currencies_array[] = array('id' => $key, 'text' => (zen_not_null($value['symbol_left']) ? $value['symbol_left'] : $value['symbol_right']), 'title' => (zen_not_null($value['title']) ? $value['title'] : '') );
			}

			$hidden_get_variables = '';
			reset($_GET);
			while (list($key, $value, $title) = each($_GET)) {
				if($key!='' && $value!='' && $title!='' ){
					if ( ($key != 'currency') && ($key != zen_session_name()) && ($key != 'x') && ($key != 'y') ) {
						$hidden_get_variables .= zen_draw_hidden_field($key, $value, $title);
					}
				}
