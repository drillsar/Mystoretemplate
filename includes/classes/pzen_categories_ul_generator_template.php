...
// Fix for line 48 provided by Paulm, uploaded by Kelvyn
 ...
 class zen_categories_ul_generator_template {
   ...
   function zen_categories_ul_generator_template($load_from_database = true) {
...
//}
   ...
   function buildBranch($parent_id, $level = 0, $submenu=false, $parent_link='') {
     $result = sprintf($this->parent_group_start_string, ($submenu==true) ? ' id="accordion-1" class="expander-list accordion level'. ($level+1) . '"' : '' );
     ...
     if (($this->data[$parent_id])) {
       foreach ($this->data[$parent_id] as $category_id => $category) {
         ...
         if (($this->data[$category_id]) && ($submenu==false)) {
           $result .= sprintf($this->parent_start_string, ($submenu==true) ? ' id="accordion-1" class="accordion level'.($level+1) . '"' : '');
		   $result .= sprintf($this->child_start_string, ($submenu==true) ? ' class="submenu"' : '');
         } else {
