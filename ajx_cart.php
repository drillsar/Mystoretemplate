 ...
 * @version $Id: ajx_cart.php 1 2014-06-05 18:53:26Z ZenExpert $
 */
	...
	if($prod){
		...
		$content='';
		$content .= '<a href="'. zen_href_link(FILENAME_SHOPPING_CART, '', 'SSL') .'" class="dropdown-toggle " id="topcartlink"><span class="badge badge--cart">'. $_SESSION['cart']->count_contents() .'</span><span class="icon icon-shopping_basket"></span><span class="name-text">'.TEXT_AJAX_CART_TITLE.'</span></a>';
		...
		if ($_SESSION['cart']->count_contents() > 0) {
			...
			$content.='<li class="cart__item">
						...
						<div class="cart__item__info">
							...
							<div class="cart__item__info__qty"><input type="text" value="'.$products[$i]['quantity'].'" class="input--ys"></div>';
							if (isset($attrArray2) && is_array($attrArray2)) {
							reset($attrArray2);
							$content.='<div class="cart__item__info__details">
								<div class="multitooltip">
									<a href="javascript:void(0);">'.TEXT_DETAILS.'</a>
		...
		$cart_top = '<a href="'. zen_href_link(FILENAME_SHOPPING_CART, '', 'SSL') .'" class="dropdown-toggle lnk-cart btn btn-blue btn-small" id="topcartlink" onClick="ajax_cart(); return false"><i class="glyphicon glyphicon-shopping-cart"></i><span class="item-count">Cart '. $_SESSION['cart']->count_contents() .'</span></a>';  


		echo json_encode(array('status'=>'success','message'=>MESSAGE_PRODUCT_ACTIONS_HAS_SUCCESS,'already'=>$alreadyincarttest,'cart'=> utf8_encode($content),'cart_top'=> utf8_encode($cart_top),'total'=> utf8_encode($currencies->format($_SESSION['cart']->show_total()))));
		
	}else{
		echo json_encode(array('status'=>'error','message'=>ZC_AJAX_ERROR_TRY_AGAIN));
	}
?>
