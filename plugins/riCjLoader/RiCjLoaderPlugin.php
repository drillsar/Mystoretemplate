...
class RiCjLoaderPlugin
{
    ...
    {    
        if($this->get('loaders') == '*') {
            ...
            */
                include DIR_WS_TEMPLATE.'auto_loaders'. '/' . $value;
            }
        }
        elseif(count($this->get('loaders')) > 0) {
            foreach($this->get('loaders') as $loader) {
                if(file_exists($path = DIR_WS_TEMPLATE.'auto_loaders'. '/loader_' . $loader .'.php')) { include $path;
                }
            }
        }
        else { 
    ...
    function findAssets($extension, $directory, $file_pattern = '', $order = 0)
    {
        ...
        foreach ($allFiles as $file) {
            ...
            elseif ($this->get('inheritance') != '' && file_exists(DIR_FS_CATALOG.DIR_WS_TEMPLATES.$this->get('inheritance').'/'.$directory.'/'.$file)) {
                $files[DIR_WS_TEMPLATE.$this->get('inheritance').'/'.$directory.'/'.$file] = $order++;
            }
        }
    ...
    function addLoaderAssets($files, $type)
    {
        $mtype = $type;
        foreach ($files as $file => $order) {
            if($mtype == 'external_jscript') { $type = str_replace('_jscript', '', $mtype); 
            }
            if($mtype == 'external_css' ) { $type = str_replace('_css', '', $mtype); 
            }
            $error = false;
            if(strlen($file) == 0) {
                $error = true;
            ...
            if($error === false) {    
                if($mtype == 'external_jscript' || $mtype == 'external_css' ) { $type = str_replace('external_', '', $mtype); 
                }
        ...
        if((is_array($this->loaders)) && count($this->loaders) > 0) {
            foreach($this->loaders as $loader){
                if(isset($loader['conditions']['pages']) && (in_array('*', $loader['conditions']['pages']) || in_array($this->current_page_base, $loader['conditions']['pages']))) {
                    ...
                    if(isset($loader['external_jscript_files'])) {
                        $this->addLoaderAssets($loader['external_jscript_files'], 'external_jscript');
                    }
                    if(isset($loader['external_css_files'])) {
                        $this->addLoaderAssets($loader['external_css_files'], 'external_css');
                    }
                }
                else{
                    $load = false;
