 ...
 * Modified for Zen Return Manager
 */
...
// check if order has open gv
        ...
        if (sizeof($order->products) > 0) {
          ...
          //bof added for return manager 1 of 1
          $orderRtn = $db->Execute("select return_id, orders_id from " . TABLE_ORDER_RETURN_MANAGER . " where orders_id = '" . (int)$oInfo->orders_id . "'"); 
          if ($orderRtn->RecordCount() > 0) {
          $contents[] = array('align' => 'center', 'text' => '<a href="' . zen_href_link(FILENAME_RETURNS, 'rID=' . $orderRtn->fields['return_id'] . '&action=edit', 'SSL') . '">' . zen_image_button('button_edit_return.png', IMAGE_EDIT) . '</a>');          
           } else {
           $contents[] = array('align' => 'center', 'text' => '<a href="' . zen_href_link(FILENAME_RETURNS,  'oID=' . $oInfo->orders_id . '&action=create_new', 'SSL') . '">' . zen_image_button('button_create_return.png', 'Create Return') . '</a>');
           }
          //eof added for return manager 1 of 1
        }
      }
      break;
  }
